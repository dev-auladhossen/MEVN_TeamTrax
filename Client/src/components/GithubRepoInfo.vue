<template>
  <div>
    <div class="relative" v-if="projectRepoInfo">
      <div
        v-if="loading"
    ...
    <div v-else class="bg-white">
      <h2 class="text-lg font-semibold text-gray-700">
        <font-awesome-icon class="text-gray-500 text-sm mr-2" icon="code" /> No
        GitHub Repository Linked Yet
      </h2>
      <p class="text-gray-700">
        This project is not connected to a GitHub repository. Please create one
        to collaborate and track code.
      </p>
      <router-link
        v-if="!githubSettingsUserName"
        to="/settings"
        class="inline-block mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 hover:text-white"
      >
        Link GitHub Account
      </router-link>
    </div>
